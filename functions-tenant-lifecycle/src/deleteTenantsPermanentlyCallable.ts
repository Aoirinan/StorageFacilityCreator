...
/**
 * Removes what is left under each deleted tenant doc: its saved cards,
 * billing/default, insurance, contact logs and payment rows. Deleting a doc
 * in a transaction leaves its subcollections behind, reachable by path but
 * by nothing in the app. Runs after the delete commits (a transaction can't
 * hold them all) and is safe to repeat: a second delete of the same ids
 * finds no tenant doc, no history, and clears whatever is still there. A
 * failure is logged, not thrown: the tenants are already gone.
 */
export async function deleteTenantSubtrees(
  db: admin.firestore.Firestore,
  facilityId: string,
  tenantIds: string[],
): Promise<void> {
  const tenants = db.collection('facilities').doc(facilityId).collection('tenants');
  for (const tenantId of tenantIds) {
    try {
      await db.recursiveDelete(tenants.doc(tenantId));
    } catch (error: unknown) {
      functions.logger.error('deleteTenantsPermanently: tenant subcollections not removed', {
        facilityId,
        tenantId,
  ...
  .https.onCall(async (data: unknown, context): Promise<DeleteTenantsResult> => {
    ...
    try {
      ...
      if (result.status === 'deleted') {
        await deleteTenantSubtrees(
          db,
          facilityId,
          result.deleted.map((d) => d.tenantId),
        );
      }
      return result;
    } catch (error: unknown) {
