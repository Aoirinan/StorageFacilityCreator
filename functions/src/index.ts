...
// SFC lead config is also declared in functions-marketing/src/secrets.ts. Both
// codebases share the same env var names; the registry lets shared helpers read
// them without forcing defineString into functions-shared (which can't have it).
...
function tenantSmsOptOutDisplayName(t: Record<string, any>): string {
...
export const listFacilitySmsOptOuts = functions.https.onCall(async (data: { facilityId?: string }, context) => {
  ...
  const fd = facilitySnap.data() as Record<string, any>;
  const blockList = Array.isArray(fd?.smsSettings?.blockList) ? (fd.smsSettings.blockList as string[]) : [];
  ...
  for (const d of qOpt.docs) byId.set(d.id, d.data() as Record<string, any>);
  for (const d of qConsent.docs) {
    if (!byId.has(d.id)) byId.set(d.id, d.data() as Record<string, any>);
  }
  ...
  for (const row of optedOutTenants) {
    const n = formatPhoneNumber(row.phone);
    if (n) tenantNormPhones.add(n);
  }
...
export const restoreFacilitySmsForPhone = functions.https.onCall(
  async (data: { facilityId?: string; phone?: string; tenantId?: string | null }, context) => {
    ...
    const fdata = facilitySnap.data() as Record<string, any>;
    const smsSettings = fdata?.smsSettings || {};
