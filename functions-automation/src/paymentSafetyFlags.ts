...
async function getPaymentSafetyConfig(): Promise<PaymentSafetyConfig> {
  try {
    const configDoc = await admin.firestore().collection('appConfig').doc('paymentSafety').get();
...
/** Same semantics as default functions bundle — used by monthly charge generation. */
