...
6. **Sample-message caveat.** Outbound bodies do not carry a brand prefix, and
   the STOP/HELP footer (`smsComplianceHelpers.ts`, `addOptOutFooter`) is only
   appended when the facility has the `enhancedOptOut` compliance flag. A
   reviewer comparing samples to real traffic will see messages with no
   brand and no opt-out line unless that flag is on. Recommended before
   sending real traffic: append the footer unconditionally and prefix the
   facility name (or "Storage Facility Creator") when sending from the shared
   toll-free number.
...
- **Sample messages** (bodies as generated by the app, with the footer the
  app appends):
  1. "Hi Jane, friendly reminder: your storage rent is due soon. Reply with any
     questions. Reply STOP to opt out. Reply HELP for help."
  2. "Jane, we show a past-due balance for unit B12. Please contact us to
     arrange payment. Thank you. Reply STOP to opt out. Reply HELP for help."
  3. "Hello Jane, your storage contract "Unit B12 lease" is scheduled to expire
     on Oct 31, 2026. Please contact us to renew or discuss next steps. Reply
...
- **Sample messages:** the three above, prefixed with the facility name, e.g.
